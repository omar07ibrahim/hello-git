...
demo_root="$repo_root/docs/demo/git-dag-v1"
report_path="$demo_root/report.html"
output_path="$repo_root/docs/assets/git-dag-report.png"
...
python3 -B "$repo_root/tools/generate_evidence.py" --write --allow-missing-screenshot >/dev/null
python3 -B "$repo_root/tools/generate_evidence.py" --check --allow-missing-screenshot >/dev/null
...
script_sha_before=$(sha256sum "$repo_root/tools/capture_report.sh" | awk '{print $1}')
...
temporary_root=$(mktemp -d "$repo_root/.git-dag-report-capture.XXXXXX")
...
capture=(
    ...
    --dump-dom
    --screenshot=/output/git-dag-report.png
...
python3 - "$repo_root" "$repo_root/evidence/git-dag-v1.json" "$dom_path" "$temporary_root/output" <<'PY'
...
from tools.generate_evidence import _validate_png
...
expected = output / "git-dag-report.png"
...
if [[ $(sha256sum "$repo_root/tools/capture_report.sh" | awk '{print $1}') != "$script_sha_before" ]]; then
...
script_path = root / "tools/capture_report.sh"
script = script_path.read_bytes()
evidence = json.loads((root / "evidence/git-dag-v1.json").read_text(encoding="utf-8"))
...
payload = {
    ...
    "input": {
        "report": {
            **row(report_path, "docs/demo/git-dag-v1/report.html"),
    ...
    "outputs": {
        "rendered_dom": row(dom_path, "docs/demo/git-dag-v1/rendered-dom.html"),
        "screenshot": {
            **row(screenshot_path, "docs/assets/git-dag-report.png"),
            "height": 1800,
            "width": 1440,
        },
    },
    "schema_version": "git-dag-browser-capture-attestation/v1",
    "script": row(script_path, "tools/capture_report.sh"),
    "viewport": {"device_scale_factor": 1, "height": 1800, "width": 1440},
}
...
python3 -B "$repo_root/tools/generate_evidence.py" --write >/dev/null
python3 -B "$repo_root/tools/generate_evidence.py" --check >/dev/null
